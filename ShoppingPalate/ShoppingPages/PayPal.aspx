...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Paypal.aspx.cs" Inherits="ShoppingPalate_ShoppingPages_PayPal" %>
...
<body onload="document.Paypal.submit();"> 
   ...
   <!-- This form contains hidden input fields with variable names required by the PayPal Payments Standard API 
    ...
    <form id="Paypal" name="Paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" >
        ...
        <!-- we do not have a paypal account hence used yours -->
        ...
        <input type="hidden" name="item_name_<%# Container.ItemIndex + 1 %>" value="<%# Eval("ProductName") %>" />
        <input type="hidden" name="quantity_<%# Container.ItemIndex + 1 %>" value="<%# Eval("Quantity") %>" />
        <input type="hidden" name="amount_<%# Container.ItemIndex + 1 %>" value="<%# Eval("ProductPrice") %>" />
